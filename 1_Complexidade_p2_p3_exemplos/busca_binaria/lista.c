...
int buscar_recursivo_penido(lista l, elem x, long ini, long fim) {
    ...
    if (l.dados[meio] < x) {
        return (meio + 1 <= fim) ? buscar_recursivo_penido(l, x, meio + 1, fim) : 0;
    } else {
        return (ini <= meio - 1) ? buscar_recursivo_penido(l, x, ini, meio - 1) : 0;
    }
}

int buscar_penido(lista l, elem x) {
    buscar_recursivo_penido(l, x, 0, l.tamanho - 1);
}
